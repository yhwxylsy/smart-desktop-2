...
> **历史留档（2026-09-06 追加）：** 本文记录的是 2026-09-05 当时的实测结果，原文不再修改。
> 此后 STM32 串口接线做了改造（USART3 全双工对 ESP32S3、SYN6288 改走 `PB3` 软件串口，
> 波特率 9600 → 115200），因此下文中 A 段的“串口 `PB11/PB10`+`PB4/PB3`+`PA3/PA2`、`9600/4800`”
> 描述**已不代表当前状态**。当前权威接线见 `docs/HARDWARE_WIRING.md`；
> 当前一致性校验请以重新运行 `python tools/verify_firmware_consistency.py` 的结果为准。
